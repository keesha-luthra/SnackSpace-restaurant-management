...
route.post('/saveDish', async (req, res) => {
    ...
    try {
        const result = await uploadToCloudinary(photo.data);
        req.body.photo = result.secure_url;
    } catch (err) {
        console.error("Cloudinary upload error:", err);
        return res.render("addNewDish", { notsave: true });
    }
...
route.post("/admin/saveEditDish/:id", async (req, res) => {
    ...
    if (req.session.loginUser) {
        ...
        if (req.files == null)
            ...
            console.log("photo is selected, uploading to Cloudinary...");
            const { photo } = req.files;
            const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/jpg', 'image/webp'];
            if (!allowedMimeTypes.includes(photo.mimetype)) {
            ...
            try {
                const result = await uploadToCloudinary(photo.data);
                req.body.photo = result.secure_url;
                console.log("Uploaded new photo:", req.body.photo);
            } catch (err) {
                console.error("Cloudinary upload error:", err);
                return res.redirect("/admin/editDish/" + req.params.id + "/error");
            }
